<?php
	...
	if(isset($_ENV['DATABASE_URL'])) {
		...
		$conf['servers'][0]['password'] = $db_config['pass'];
		$db_count = 0;
		$keys = preg_grep('/^HEROKU_POSTGRESQL_[A-Za-z]+_URL$/i',array_keys($_ENV));
		foreach($keys as $k)
		{
			$db_count++;
			$db_config = parse_url($_ENV[$k]);
			$conf['servers'][$db_count]['desc'] = $k;
			$conf['servers'][$db_count]['host'] = $db_config['host'];
